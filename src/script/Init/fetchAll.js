...
export const getCountryAmount = (amount,startDate, endDate, country, isShow) => {
    ...
    const url = `https://www.statbureau.org/${TYPE_OF_FETCH}?jsoncallback=?&country=${encodeURIComponent(country)}&start=${encodeURIComponent(startDate)}&end=${encodeURIComponent(endDate)}&amount=${encodeURIComponent(amount)}&format=true`;
console.log(url)
